...
const filepath = path.join(__dirname, 'chatbot_widget.js');
let content = fs.readFileSync(filepath, 'utf8');

const new_getAIResponse = `
...
<div class="quick-chips-container" style="margin-top: 15px;">
    ...
    <button class="quick-chip" onclick="sendQuickKeyword('연락처')">📞 전화 문의</button>
        ...
        switch (maxIntent) {
            ...
            case 'ceo':
                return { text: "저희 비에이텍의 대표이사는 **조세형** 대표님입니다. 최고의 물 관리 기술력으로 보답하겠습니다.<br>\${getFollowUpPrompt()}", type: 'ai' };
            case 'location':
                return { text: "비에이텍 본사 및 제1공장은 **강원특별자치도 춘천시 퇴계공단2길 64**에 위치해 있습니다. 내비게이션에 '비에이텍'을 검색하시면 편리합니다.<br>\${getFollowUpPrompt()}", type: 'ai', source: '오시는 길 안내' };
            case 'company':
                return { text: "(주)비에이텍은 깨끗하고 안전한 물 환경을 조성하는 워터펌프 시스템 전문 기업입니다. 우수한 기술력으로 상하수도 설비, 공업용수, 부스터 펌프 시스템 등을 제조합니다.<br>\${getFollowUpPrompt()}", type: 'ai' };
            case 'products':
                chatState = 'prod_function_select';
                return { text: "어떤 용도의 펌프를 찾으시나요? 아래 메뉴를 클릭하거나 번호를 입력해 주세요.<br><div class='quick-chips-container' style='margin-top: 10px;'><button class='quick-chip' onclick='sendQuickKeyword(\"1\")'>1. 건물/급수용 부스터</button><button class='quick-chip' onclick='sendQuickKeyword(\"2\")'>2. 수중 오폐수 배수펌프</button><button class='quick-chip' onclick='sendQuickKeyword(\"3\")'>3. 고농도 슬러지 펌프</button><button class='quick-chip' onclick='sendQuickKeyword(\"4\")'>4. 초고점도 모노펌프</button><button class='quick-chip' onclick='sendQuickKeyword(\"5\")'>5. 다목적 순환 볼류트펌프</button><button class='quick-chip' onclick='sendQuickKeyword(\"6\")'>6. 초정밀 약품 정량펌프</button></div>", type: 'ai' };
            case 'diagnostics':
                chatState = 'diag_pump_select';
                return { text: "어떤 펌프에서 문제가 발생했나요? 아래 메뉴를 누르시거나 번호를 입력해 주시면 고장 증상 및 조치 방법을 진단해 드립니다.<br><div class='quick-chips-container' style='margin-top: 10px;'><button class='quick-chip' onclick='sendQuickKeyword(\"1\")'>1. 부스터 펌프</button><button class='quick-chip' onclick='sendQuickKeyword(\"2\")'>2. 수중 펌프</button><button class='quick-chip' onclick='sendQuickKeyword(\"3\")'>3. 슬러지 펌프</button><button class='quick-chip' onclick='sendQuickKeyword(\"4\")'>4. 모노 펌프</button><button class='quick-chip' onclick='sendQuickKeyword(\"5\")'>5. 볼류트 펌프</button><button class='quick-chip' onclick='sendQuickKeyword(\"6\")'>6. 정량 펌프</button></div>", type: 'ai' };
            case 'contact':
                return { text: "고객지원 대표전화는 **033-264-9240** 입니다. 전문가가 신속하게 도와드리겠습니다.<br>\${getFollowUpPrompt()}", type: 'ai' };
            case 'rnd':
                return { text: "비에이텍 연구소는 3D CFD(전산유체역학) 유동해석 기술을 통해 최적의 펌프 임펠러 및 케이싱을 독자 설계합니다. 에너지 효율을 극대화하는 인버터 기술력도 보유하고 있습니다.<br>\${getFollowUpPrompt()}", type: 'ai' };
            case 'certifications':
                return { text: "비에이텍은 ISO 9001 품질경영인증, 고효율 에너지 기자재 인증, KC 안전인증을 비롯하여 펌프 설비 관련 다수의 벤처 특허와 기술 인증을 보유하고 있습니다.<br>\${getFollowUpPrompt()}", type: 'ai' };
            case 'facilities':
                return { text: "최신식 CNC 머시닝센터, 플라즈마 절단기, 전용 대형 호이스트 크레인 등 펌프의 생산부터 가공, 테스트 조립까지 완벽한 자체 스마트 공장 라인을 갖추고 있습니다.<br>\${getFollowUpPrompt()}", type: 'ai' };
            case 'regulations':
                return { text: "사내 규정 및 인사/포상 서식 관련 안내는 사내 포털의 '서식 및 자료실' 페이지에 업로드되어 있습니다. (NotebookLM 가이드 등)<br>\${getFollowUpPrompt()}", type: 'ai' };
            default:
...
const regex = /\/\/ 9\. 웹페이지 지식망 & 대표전화 매핑 라우터.*?function getAIResponse.*?return \{ text: repContactTemplate, type: 'ai' \};\s*\}\s*\}/s;
content = content.replace(regex, new_getAIResponse);

// Update appendMessage to support typing effect
const appendRegex = /function appendMessage\(text, type, source = null\) \{.*?\n\s+chatBody\.appendChild\(bubble\);\s*\}/s;
const newAppendMessage = `
...
`;
content = content.replace(appendRegex, newAppendMessage.trim());

fs.writeFileSync(filepath, content, 'utf8');
console.log('Successfully updated chatbot_widget.js with NLP intents and typing effects.');
