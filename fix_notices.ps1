...
# --- 1. portal.html Dashboard Notice Loader Fix ---
$portalHtml = [IO.File]::ReadAllText("portal.html", $utf8)
$portalHtml = $portalHtml -replace '(?s)function loadDashNotices\(\) \{.*?(?=function loadDashScheds\(\) \{)', '
...
$noticesContent = '
    ...
    <dialog id="notice-detail" style="border: none; border-radius: 12px; padding: 2rem; box-shadow: 0 10px 25px rgba(0,0,0,0.2); max-width: 700px; width: 100%;">
        ...
        <div style="display: flex; justify-content: space-between; align-items: center;">
            <div>
                <button class="btn-sm" style="background:#fee2e2; color:#991b1b; margin-right:5px;" onclick="deleteNotice()">삭제</button>
                <button class="btn-sm" style="background:#e0f2fe; color:#0284c7;" onclick="editNotice()">수정</button>
            </div>
    ...
    <script>
        function initDB() {
            if(!localStorage.getItem("batech_notices_init")) {
                ...
                // Clear old custom notices to prevent duplicates during this upgrade
        ...
        function openNotice(id) {
            ...
            if(n) {
                ...
                // Format content cleanly. If it has HTML tags, render as HTML. If text, convert newlines to <br>
        ...
        function deleteNotice() {
            ...
            if(!id) return;
            const pwd = prompt("해당 공지사항을 삭제하시려면 관리자 암호를 입력하세요:");
            if(pwd === "1109") {
        ...
        function editNotice() {
            let id = document.getElementById("nd-current-id").value;
            if(!id) return;
            const pwd = prompt("해당 공지사항을 수정하시려면 관리자 암호를 입력하세요:");
            if(pwd === "1109") {
...
$writeContent = '
    ...
    <script>
        document.addEventListener("DOMContentLoaded", () => {
            ...
            if(editId) {
                ...
                if(n) {
                    ...
                    // convert HTML <p>, <br> back to newlines for editor readability
        ...
        function submitNotice() {
            ...
            // Convert newlines to HTML <p> or <br> for styling
