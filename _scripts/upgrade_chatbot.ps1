$content = Get-Content ".\chatbot_widget.js" -Raw -Encoding UTF8
...
$content = $content -replace '(?s)// 9\. 웹페이지 지식망 & 대표전화 매핑 라우터.*?function getAIResponse.*?return \{ text: repContactTemplate, type: ''ai'' \};\s*\}\s*\}', $new_getAIResponse

$content = $content -replace '(?s)function appendMessage\(text, type, source = null\) \{.*?\n\s+chatBody\.appendChild\(bubble\);\s*\}', $new_appendMessage

Set-Content -Path ".\chatbot_widget.js" -Value $content -Encoding UTF8
Write-Output "Successfully updated chatbot_widget.js via PowerShell"
