...
foreach ($file in $files) {
    ...
    if ($content -match '(?s)<nav id="navbar".*?</nav>') {
        $oldNav = $Matches[0]
        $content = $content.Replace($oldNav, $navbarHtml)
